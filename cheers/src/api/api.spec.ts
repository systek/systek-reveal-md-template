...
import { paths, __secretOrderUrl } from "./paths";
import { shape } from "./_mock_/test.utils";
    ...
    describe.each([
      ...
      (storedOrder, placedOrder, expected) => {
        ...
        beforeEach(() => {
          ...
          post(__secretOrderUrl, placedOrder)
            .then((e) => (response = e))
    ...
    describe.each([
      ...
      (storedOrder, placedOrder, expected) => {
        ...
        it(`should store total order ${shape(expected)}`, () => {
          ...
          expect(mocks.set).toHaveBeenCalledWith(__secretOrderUrl, expected);
        });
      }
    );
    ...
    describe.each([
      ...
      (placedOrder, expected) => {
        ...
        beforeEach(() => {
          ...
          post(__secretOrderUrl, placedOrder).catch((e) => (response = e));
        });
